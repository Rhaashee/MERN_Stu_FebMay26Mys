...
function rejectRequests(req,res,next){
    try {
        ...
        const requests = connectionsData.requests.filter(user=> user.id==requestId);
...
function getRequests(req,res,next){
    try {
        ...
        const connections = connectionsData.connections.filter(user=> user.user1==userId||user.user2==userId);
