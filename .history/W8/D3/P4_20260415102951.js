...
async function main(){
    try {
        ...
        const User1 = await UserRef.create({name:"Rhaashee"
        });

        const Order = await OrderRef.create(
            {product:"Phone",price:60000,user:refUser._id},{product:"Dyson hairSetting tool",price:50000,user:refUser._id},{product:"Airpods",price:20000,user:refUser._id}
        );
