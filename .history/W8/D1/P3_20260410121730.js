...
async function runFilterDemo() {
    try {
        ...
        const equalQuery = await Product.find({staus:{$eq:"active"}});
        console.log("Products which are active",equalQuery);

        const greaterQuery = await Product.find({price:{$gt:15000}});
        console.log("Products which are above ₹15000: ",greaterQuery);

        const const greaterQuery = await Product.find({price:{$gt:15000}});
        console.log("Products which are above ₹15000: ",greaterQuery); = await Product.find({price:{$ne:15000}});
        console.log("Products which are above ₹15000: ",greaterQuery);
